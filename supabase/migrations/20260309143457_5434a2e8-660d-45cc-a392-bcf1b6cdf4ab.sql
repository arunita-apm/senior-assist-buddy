...
CREATE POLICY "Caregiver can view patient medications" ON public.medications FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.users WHERE users.id = medications.user_id AND users.caregiver_email = (SELECT email FROM auth.users WHERE auth.users.id = auth.uid())::text));
...
CREATE POLICY "Caregiver can view patient reminders" ON public.reminders FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.users WHERE users.id = reminders.user_id AND users.caregiver_email = (SELECT email FROM auth.users WHERE auth.users.id = auth.uid())::text));
...
CREATE POLICY "Caregiver can view patient appointments" ON public.appointments FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.users WHERE users.id = appointments.user_id AND users.caregiver_email = (SELECT email FROM auth.users WHERE auth.users.id = auth.uid())::text));
